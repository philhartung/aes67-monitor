  ...
  return new Promise((resolve, reject) => {
    ...
    const browser = bonjour.find({ type: 'nmos-node' }, async (service) => {
      for (const address of service.addresses) {
        try {
          const response = await fetch(`http://${address}:${service.port}/x-nmos/node/v1.0/senders`);
          if (!response.ok) {
          ...
          for (const sender of json) {
            if (sender.manifest_href) {
              ...
              if (!manifestResponse.ok) {
                throw new Error('HTTP Error downloading manifest_href: ' + manifestResponse.statusText);
              }
